...
! Computation of the energy associated to the density matrix
  ...
  WRITE(*,*)'E(D_n)=',ETOT
  CALL OUTPUT_ITER(ITER,MAXITR,PDM,PHI,NBAST,EIG,EIGVEC,ETOT)
...
5 CALL OUTPUT_FINALIZE(ITER,MAXITR,PDM,PHI,NBAST,EIG,EIGVEC,ETOT)
...
! Assembly of the Fock matrices
  ...
  CALL OUTPUT_ITER(ITER,MAXITR,PTDM,PHI,NBAST,EIG,EIGVEC,ETOT)
! Numerical convergence check
...
3 WRITE(*,*)' ' ; WRITE(*,*)'Subroutine ROOTHAAN: no convergence after',ITER,'iteration(s).'
  ...
  GO TO 5
4 WRITE(*,*)'(called from subroutine ROOTHAAN)'
5 CALL OUTPUT_FINALIZE(ITER,MAXITR,PTDM,PHI,NBAST,EIG,EIGVEC,ETOT)
...
! Assembly of the Fock matrices for closed and open-shell orbitals
  ...
  ! TODO: CHECK PDMC+PDMO
  CALL OUTPUT_ITER(ITER,MAXITR,PDMC+PDMO,PHI,NBAST,EIG,EIGVEC,ETOT)
! Numerical convergence check
...
3 WRITE(*,*)'Subroutine ROOTHAAN: no convergence after',ITER,'iteration(s).'
  ...
  GO TO 5
4 WRITE(*,*)'(called from subroutine ROOTHAAN)'
5 CALL OUTPUT_FINALIZE(ITER,MAXITR,PDMC+PDMO,PHI,NBAST,EIG,EIGVEC,ETOT)
