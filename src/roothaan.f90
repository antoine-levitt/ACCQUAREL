...
! Assembly of the Fock matrices
  ...
  CALL OUTPUT_ITER(ITER,PTDM,PHI,NBAST,EIG,EIGVEC,ETOT)
! Numerical convergence check
  CALL CHECKNUMCONV(PDMA,PDMB,PTDM1,PFMA,PFMB,NBAST,ETOT,ETOT1,TRSHLD,NUMCONV,.TRUE.)
  IF (NUMCONV) THEN
! Convergence reached
     GO TO 2
  ELSE IF (ITER==MAXITR) THEN
...
3 WRITE(*,*)' ' ; WRITE(*,*)'Subroutine ROOTHAAN: no convergence after',ITER,'iteration(s).'
  ...
  GO TO 5
4 WRITE(*,*)'(called from subroutine ROOTHAAN)'
5 CALL OUTPUT_FINALIZE(ITER,PTDM,PHI,NBAST,EIG,EIGVEC,ETOT)
...
! Assembly of the Fock matrices for closed and open-shell orbitals
  ...
  CALL OUTPUT_ITER(ITER,PDMC+PDMO,PHI,NBAST,EIG,EIGVEC,ETOT)
! Numerical convergence check
  CALL CHECKNUMCONV(PDMC,PDMO,PDMC1,PDMO1,PFMC,PFMO,NBAST,ETOT,ETOT1,TRSHLD,NUMCONV,.TRUE.)
  IF (NUMCONV) THEN
! Convergence reached
     GO TO 2
  ELSE IF (ITER==MAXITR) THEN
