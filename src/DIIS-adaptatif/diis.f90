...
! DIIS with hard restart when linear dependencies encountered and with adaptative linear dependence parameter
...
!  READ(100,'(/,f16.8)')MXDIV !parameter to check the independence of vectors of Gram matrix
  READ(100,'(/,i2)')NPE !number of electrons pair 
  CLOSE(100)
...
! we have reached the maximum allowed dimension for the simplex
        PDMSET=0.D0 ! elimination of all the stored density matrix
        ERRSET=0.D0 ! elimination of all the stored error vector
	YRTGSET=0.D0
	MSET=1
...
! assembly of the pseudo-density matrix
     ...
     ELSE
!general formula
	YRTGSET(MSET-1,:)=ERRSET(MSET,:)-ERRSET(1,:)
	DO I=1,MSET-2
        YRTGSET(MSET-1,:)=YRTGSET(MSET-1,:)-DOT_PRODUCT(YRTGSET(I,:),ERRSET(MSET,:)-ERRSET(1,:))&
&/(NORM2(YRTGSET(I,:))**2)*YRTGSET(I,:)
	MXDIV=NORM2(ERRSET(1,:))**(2./(MSET-1))
	END DO
...
WRITE(*,*)'Y-QY NORM =',NORM2(YRTGSET(MSET-1,:))
	     IF (MXDIV*NORM2(ERRSET(MSET,:)-ERRSET(1,:)) > NORM2(YRTGSET(MSET-1,:))) THEN
