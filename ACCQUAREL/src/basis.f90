...
! note: the coefficients in the contractions are derived from the RKB scheme and put to 1 if the UKB scheme is used.
                 DO K=1,NGTOLSC(I)
                    ...
                    DO L=IP,IP+NBOPIC-1
                       ...
                       ELSE
                          CGTO(ILGTO)%coefficients(IPIC)=2.D0*ALPHA(L,I,M)*CCOEF(L,J,I,M)
                       END IF
                       CGTO(ILGTO)%monomialdegree=MDLSC(I,K)
                    END DO
                 END DO
...
3 IDX=ILGTO-NGTOLSC(I)
  SELECT CASE (I)
     ...
     SELECT CASE (K)
        ...
        PHI(ILA)%coefficients(1,1)=(-1.D0,0.D0) ; PHI(ILA)%coefficients(2,1:3)=(/(-1.D0,0.D0),(0.D0,-1.D0),(0.D0,2.D0)/)
        PHI(ILB)%nbrofcontractions=(/3,1/)
        PHI(ILB)%contractions(1,1:3)=(/CGTO(IDX+4),CGTO(IDX+7),CGTO(IDX+12)/) ; PHI(ILB)%contractions(2,1)=CGTO(IDX+8)
        PHI(ILB)%contidx(1,1:3)=(/IDX+4,IDX+7,IDX+12/) ; PHI(ILB)%contidx(2,1)=IDX+8
        PHI(ILB)%coefficients(1,1:3)=(/(-1.D0,0.D0),(0.D0,1.D0),(0.D0,-2.D0)/) ; PHI(ILB)%coefficients(2,1)=(1.D0,0.D0)
        CASE (3)
        PHI(ILA)%nbrofcontractions=(/2,2/)
        ...
        PHI(ILB)%nbrofcontractions=(/2,2/)
        PHI(ILB)%contractions(1,1:2)=(/CGTO(IDX+5),CGTO(IDX+9)/) ; PHI(ILB)%contractions(2,1:2)=(/CGTO(IDX+10),CGTO(IDX+13)/)
        PHI(ILB)%contidx(1,1:2)=(/IDX+5,IDX+9/) ; PHI(ILB)%contidx(2,1:2)=(/IDX+10,IDX+13/)
        ...
        PHI(ILB)%contractions(1,1:4)=(/CGTO(IDX+2),CGTO(IDX+4),CGTO(IDX+11),CGTO(IDX+12)/) ; PHI(ILB)%contractions(2,1)=CGTO(IDX+6)
        PHI(ILB)%contidx(1,1:4)=(/IDX+2,IDX+4,IDX+11,IDX+12/) ; PHI(ILB)%contidx(2,1)=IDX+6
        ...
        PHI(ILB)%contractions(1,1:3)=(/CGTO(IDX+3),CGTO(IDX+6),CGTO(IDX+13)/)
        PHI(ILB)%contractions(2,1:2)=(/CGTO(IDX+5),CGTO(IDX+11)/)
        PHI(ILB)%contidx(1,1:3)=(/IDX+3,IDX+6,IDX+13/) ; PHI(ILB)%contidx(2,1:2)=(/IDX+5,IDX+11/)
        ...
        PHI(ILA)%coefficients(1,1:2)=(/(-1.D0,0.D0),(1.D0,0.D0)/)
        PHI(ILA)%coefficients(2,1:3)=(/(-1.D0,0.D0),(0.D0,-1.D0),(0.D0,1.D0)/)
        PHI(ILB)%nbrofcontractions=(/3,2/)
        PHI(ILB)%contractions(1,1:3)=(/CGTO(IDX+6),CGTO(IDX+8),CGTO(IDX+13)/)
        PHI(ILB)%contractions(2,1:2)=(/CGTO(IDX+9),CGTO(IDX+12)/)
        PHI(ILB)%contidx(1,1:3)=(/IDX+6,IDX+8,IDX+13/) ; PHI(ILB)%contidx(2,1:2)=(/IDX+9,IDX+12/)
        PHI(ILB)%coefficients(1,1:3)=(/(-1.D0,0.D0),(0.D0,1.D0),(0.D0,-1.D0)/)
        PHI(ILB)%coefficients(2,1:2)=(/(1.D0,0.D0),(-1.D0,0.D0)/)
     END SELECT
     CASE (4)
     SELECT CASE (K)
        ...
        PHI(ILA)%coefficients(1,1)=(-1.D0,0.D0) ; PHI(ILA)%coefficients(2,1:3)=(/(-1.D0,0.D0),(0.D0,-1.D0),(3.D0,0.D0)/)
        PHI(ILB)%nbrofcontractions=(/3,1/)
        PHI(ILB)%contractions(1,1:3)=(/CGTO(IDX+1),CGTO(IDX+2),CGTO(IDX+16)/) ; PHI(ILB)%contractions(2,1)=CGTO(IDX+3)
        PHI(ILB)%contidx(1,1:3)=(/IDX+1,IDX+2,IDX+16/) ; PHI(ILB)%contidx(2,1)=IDX+3
        PHI(ILB)%coefficients(1,1:3)=(/(-1.D0,0.D0),(0.D0,1.D0),(3.D0,0.D0)/) ; PHI(ILB)%coefficients(2,1)=(1.D0,0.D0)
        CASE (2)
        PHI(ILA)%nbrofcontractions=(/1,4/)
        ...
        PHI(ILA)%coefficients(1,1)=(-1.D0,0.D0)
        PHI(ILA)%coefficients(2,1:4)=(/(-1.D0,0.D0),(0.D0,-1.D0),(0.D0,1.D0),(2.D0,0.D0)/)
        PHI(ILB)%nbrofcontractions=(/4,1/)
        PHI(ILB)%contractions(1,1:4)=(/CGTO(IDX+2),CGTO(IDX+4),CGTO(IDX+16),CGTO(IDX+17)/)
        PHI(ILB)%contractions(2,1)=CGTO(IDX+5)
        PHI(ILB)%contidx(1,1:4)=(/IDX+2,IDX+4,IDX+16,IDX+17/) ; PHI(ILB)%contidx(2,1)=IDX+5
        PHI(ILB)%coefficients(1,1:4)=(/(-1.D0,0.D0),(0.D0,1.D0),(0.D0,-1.D0),(2.D0,0.D0)/)
        PHI(ILB)%coefficients(2,1)=(1.D0,0.D0)
        CASE (3)
        PHI(ILA)%nbrofcontractions=(/2,3/)
        ...
        PHI(ILA)%coefficients(1,1:2)=(/(-1.D0,0.D0),(1.D0,0.D0)/)
        PHI(ILA)%coefficients(2,1:3)=(/(-1.D0,0.D0),(0.D0,-1.D0),(2.D0,0.D0)/)
        PHI(ILB)%nbrofcontractions=(/3,2/)
        PHI(ILB)%contractions(1,1:3)=(/CGTO(IDX+3),CGTO(IDX+5),CGTO(IDX+18)/)
        PHI(ILB)%contractions(2,1:2)=(/CGTO(IDX+6),CGTO(IDX+16)/)
        PHI(ILB)%contidx(1,1:3)=(/IDX+3,IDX+5,IDX+18/) ; PHI(ILB)%contidx(2,1:2)=(/IDX+6,IDX+16/)
        PHI(ILB)%coefficients(1,1:3)=(/(-1.D0,0.D0),(0.D0,1.D0),(2.D0,0.D0)/)
        PHI(ILB)%coefficients(2,1:2)=(/(1.D0,0.D0),(-1.D0,0.D0)/)
        CASE (4)
        PHI(ILA)%nbrofcontractions=(/1,4/)
        ...
        PHI(ILA)%coefficients(1,1)=(-1.D0,0.D0) ; PHI(ILA)%coefficients(2,1:4)=(/(-1.D0,0.D0),(0.D0,-1.D0),(0.D0,2.D0),(1.D0,0.D0)/)
        PHI(ILB)%nbrofcontractions=(/4,1/)
        PHI(ILB)%contractions(1,1:4)=(/CGTO(IDX+4),CGTO(IDX+7),CGTO(IDX+17),CGTO(IDX+19)/) ; PHI(ILB)%contractions(2,1)=CGTO(IDX+8)
        PHI(ILB)%contidx(1,1:4)=(/IDX+4,IDX+7,IDX+17,IDX+19/) ; PHI(ILB)%contidx(2,1)=IDX+8
        PHI(ILB)%coefficients(1,1:4)=(/(-1.D0,0.D0),(0.D0,1.D0),(0.D0,-2.D0),(1.D0,0.D0)/) ; PHI(ILB)%coefficients(2,1)=(1.D0,0.D0)
        CASE (5)
        PHI(ILA)%nbrofcontractions=(/2,3/)
        ...
        PHI(ILA)%coefficients(1,1:2)=(/(-1.D0,0.D0),(2.D0,0.D0)/)
        PHI(ILA)%coefficients(2,1:3)=(/(-1.D0,0.D0),(0.D0,-1.D0),(1.D0,0.D0)/)
        PHI(ILB)%nbrofcontractions=(/3,2/)
        PHI(ILB)%contractions(1,1:3)=(/CGTO(IDX+6),CGTO(IDX+9),CGTO(IDX+21)/)
        PHI(ILB)%contractions(2,1:2)=(/CGTO(IDX+10),CGTO(IDX+18)/)
        PHI(ILB)%contidx(1,1:3)=(/IDX+6,IDX+9,IDX+21/) ; PHI(ILB)%contidx(2,1:2)=(/IDX+10,IDX+18/)
        PHI(ILB)%coefficients(1,1:3)=(/(-1.D0,0.D0),(0.D0,1.D0),(1.D0,0.D0)/)
        PHI(ILB)%coefficients(2,1:2)=(/(1.D0,0.D0),(-2.D0,0.D0)/)
        CASE (6)
        ...
        PHI(ILA)%coefficients(1,1:2)=(/(-1.D0,0.D0),(1.D0,0.D0)/)
        PHI(ILA)%coefficients(2,1:4)=(/(-1.D0,0.D0),(0.D0,-1.D0),(0.D0,1.D0),(1.D0,0.D0)/)
        PHI(ILB)%nbrofcontractions=(/4,2/)
        PHI(ILB)%contractions(1,1:4)=(/CGTO(IDX+5),CGTO(IDX+8),CGTO(IDX+18),CGTO(IDX+20)/)
        PHI(ILB)%contractions(2,1:2)=(/CGTO(IDX+9),CGTO(IDX+17)/)
        PHI(ILB)%contidx(1,1:4)=(/IDX+5,IDX+8,IDX+18,IDX+20/) ; PHI(ILB)%contidx(2,1:2)=(/IDX+9,IDX+17/)
        PHI(ILB)%coefficients(1,1:4)=(/(-1.D0,0.D0),(0.D0,1.D0),(0.D0,-1.D0),(1.D0,0.D0)/)
        PHI(ILB)%coefficients(2,1:2)=(/(1.D0,0.D0),(-1.D0,0.D0)/)
        CASE (7)
        PHI(ILA)%nbrofcontractions=(/1,3/)
        ...
        PHI(ILA)%coefficients(1,1)=(-1.D0,0.D0) ; PHI(ILA)%coefficients(2,1:3)=(/(-1.D0,0.D0),(0.D0,-1.D0),(0.D0,3.D0)/)
        PHI(ILB)%nbrofcontractions=(/3,1/)
        PHI(ILB)%contractions(1,1:3)=(/CGTO(IDX+10),CGTO(IDX+11),CGTO(IDX+19)/) ; PHI(ILB)%contractions(2,1)=CGTO(IDX+12)
        PHI(ILB)%contidx(1,1:3)=(/IDX+10,IDX+11,IDX+19/) ; PHI(ILB)%contidx(2,1)=IDX+12
        PHI(ILB)%coefficients(1,1:3)=(/(-1.D0,0.D0),(0.D0,1.D0),(0.D0,-3.D0)/) ; PHI(ILB)%coefficients(2,1)=(1.D0,0.D0)
        CASE (8)
        PHI(ILA)%nbrofcontractions=(/2,3/)
        ...
        PHI(ILA)%coefficients(1,1:2)=(/(-1.D0,0.D0),(1.D0,0.D0)/)
        PHI(ILA)%coefficients(2,1:3)=(/(-1.D0,0.D0),(0.D0,-1.D0),(0.D0,2.D0)/)
        PHI(ILB)%nbrofcontractions=(/3,2/)
        PHI(ILB)%contractions(1,1:3)=(/CGTO(IDX+8),CGTO(IDX+12),CGTO(IDX+20)/)
        PHI(ILB)%contractions(2,1:2)=(/CGTO(IDX+13),CGTO(IDX+19)/)
        PHI(ILB)%contidx(1,1:3)=(/IDX+8,IDX+12,IDX+20/) ; PHI(ILB)%contidx(2,1:2)=(/IDX+13,IDX+19/)
        PHI(ILB)%coefficients(1,1:3)=(/(-1.D0,0.D0),(0.D0,1.D0),(0.D0,-2.D0)/)
        PHI(ILB)%coefficients(2,1:2)=(/(1.D0,0.D0),(-1.D0,0.D0)/)
        CASE (9)
        PHI(ILA)%nbrofcontractions=(/2,3/)
        ...
        PHI(ILA)%coefficients(2,1:3)=(/(-1.D0,0.D0),(0.D0,-1.D0),(0.D0,1.D0)/)
        PHI(ILB)%nbrofcontractions=(/3,2/)
        PHI(ILB)%contractions(1,1:3)=(/CGTO(IDX+9),CGTO(IDX+13),CGTO(IDX+21)/)
        PHI(ILB)%contractions(2,1:2)=(/CGTO(IDX+14),CGTO(IDX+20)/)
        PHI(ILB)%contidx(1,1:3)=(/IDX+9,IDX+13,IDX+21/) ; PHI(ILB)%contidx(2,1:2)=(/IDX+14,IDX+20/)
        PHI(ILB)%coefficients(1,1:3)=(/(-1.D0,0.D0),(0.D0,1.D0),(0.D0,-1.D0)/)
        PHI(ILB)%coefficients(2,1:2)=(/(1.D0,0.D0),(-2.D0,0.D0)/)
        CASE (10)
        PHI(ILA)%nbrofcontractions=(/2,2/)
        ...
        PHI(ILA)%coefficients(1,1:2)=(/(-1.D0,0.D0),(3.D0,0.D0)/) ; PHI(ILA)%coefficients(2,1:2)=(/(-1.D0,0.D0),(0.D0,-1.D0)/)
        PHI(ILB)%nbrofcontractions=(/2,2/)
        PHI(ILB)%contractions(1,1:2)=(/CGTO(IDX+10),CGTO(IDX+14)/) ; PHI(ILB)%contractions(2,1:2)=(/CGTO(IDX+15),CGTO(IDX+21)/)
        PHI(ILB)%contidx(1,1:2)=(/IDX+10,IDX+14/) ; PHI(ILB)%contidx(2,1:2)=(/IDX+15,IDX+21/)
        PHI(ILB)%coefficients(1,1:2)=(/(-1.D0,0.D0),(0.D0,1.D0)/) ; PHI(ILB)%coefficients(2,1:2)=(/(1.D0,0.D0),(-3.D0,0.D0)/)
     END SELECT
  END SELECT
  IF (UNCONT) THEN
     GO TO 1
